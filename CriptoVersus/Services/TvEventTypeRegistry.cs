...
public static class TvEventTypeRegistry
{
    private static readonly IReadOnlyDictionary<string, TvEventTypeDefinition> Definitions =
        new Dictionary<string, TvEventTypeDefinition>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["candle-battle-lead-change"] = new("candle-battle-lead-change", "VIRADA DO CANDLE BATTLE", "CANDLE BATTLE LEAD CHANGE", "{TEAM} virou a liderança no Candle Battle.", "{TEAM} took the Candle Battle lead.", "/images/tv/events/candle-battle-lead-change.png", "⚔", "#53c8ff", "#ff9f43", "rgba(83, 200, 255, 0.52)", "pulse-surge", "goal", 5)
        };
    ...
    public static TvEventTypeDefinition Resolve(MatchScoreEventDto? item)
    {
        ...
        if (combined.Contains("CANDLE", StringComparison.Ordinal) || combined.Contains("LEAD_CHANGE", StringComparison.Ordinal) || combined.Contains("CANDLE_BATTLE", StringComparison.Ordinal)) return Definitions["candle-battle-lead-change"];
