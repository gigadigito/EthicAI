...
export type FuturebolBoundaryKind = "InPlay" | "Goal" | "Out";

export interface FuturebolBoundaryResult {
    kind: FuturebolBoundaryKind;
    scoringTeam: FuturebolTeam | null;
    restartingTeam: FuturebolTeam | null;
    restartType: FuturebolRestartType | null;
}

const IN_PLAY: FuturebolBoundaryResult = Object.freeze({
    kind: "InPlay",
    scoringTeam: null,
    restartingTeam: null,
    restartType: null
...
export class FuturebolMatchRules {
    public evaluateBoundary(
        previous: FuturebolVector3State,
        current: FuturebolVector3State,
        lastTouchTeam: FuturebolTeam | null
    ): FuturebolBoundaryResult {
        const crossedPositiveGoalLine =
            previous.x <= FUTUREBOL_FIELD.goalLineX &&
            current.x > FUTUREBOL_FIELD.goalLineX;
        const crossedNegativeGoalLine =
            previous.x >= -FUTUREBOL_FIELD.goalLineX &&
            current.x < -FUTUREBOL_FIELD.goalLineX;
        const insideGoal =
            Math.abs(current.z) < FUTUREBOL_FIELD.goalHalfWidth - 0.08 &&
        ...
        const defendingTeam: FuturebolTeam = current.x > 0 ? "away" : "home";
