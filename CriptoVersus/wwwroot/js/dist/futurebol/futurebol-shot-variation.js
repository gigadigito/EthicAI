 ...
 */
export function selectShotProfile(attackerZ, attackDir, advantage, seed, salt) {
...
function selectLateral(attackerZ, attackDir, seed, salt) {
...
function selectHeight(seed, salt) {
    const roll = deterministicUnit(seed, salt);
    if (roll < 0.55)
        return "Low";
    if (roll < 0.82)
...
function selectPower(advantage, seed, salt) {
 ...
 */
export function computeShotTarget(profile, attackDir, shotPlacement) {
...
function computeLateralZ(lateral, shotPlacement, goalHalfWidth) {
    switch (lateral) {
        case "NearPost":
            return clamp(shotPlacement * 0.3, -goalHalfWidth + 0.5, goalHalfWidth - 0.5);
        case "FarPost":
            return clamp(Math.sign(shotPlacement) * goalHalfWidth * 0.85 + shotPlacement * 0.15, -goalHalfWidth + 0.5, goalHalfWidth - 0.5);
        case "Center":
            return clamp(shotPlacement * 0.1, -goalHalfWidth + 1.0, goalHalfWidth - 1.0);
    }
}
function computeHeightY(height) {
    switch (height) {
...
function computePowerOffset(power) {
    return power * 1.35;
}
/**
 ...
 */
export function computeGoalkeeperDiveTarget(shotEndZ, goalkeeperZ, _attackDir) {
    const lateralDiff = shotEndZ - goalkeeperZ;
    const targetZ = goalkeeperZ + lateralDiff * 0.75;
    const diveDirection = lateralDiff > 0.3 ? "right" :
