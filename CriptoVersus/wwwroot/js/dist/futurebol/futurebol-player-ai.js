export class FuturebolPlayerAI {
    decide(context) {
        ...
        const nearestOpponent = minimumDistance(context.owner.position, context.opponents);
        const pass = this.selectPassReceiver(context);
        const shotLaneRisk = laneRisk(context.owner.position, { x: goalX, y: 1, z: 0 }, context.opponents);
        ...
        const spaceAhead = forwardSpace(context.owner.position, direction, context.opponents);
        if (spaceAhead >= 3.2)
    ...
    selectPassReceiver(context) {
        ...
        for (const teammate of context.teammates) {
            ...
            if (distance < 2.5 || distance > 22)
                continue;
            const progress = (target.x - context.owner.position.x) * direction;
            const openness = minimumDistance(target, context.opponents);
            const risk = laneRisk(context.owner.position, target, context.opponents);
            const rolePenalty = teammate.role === "goalkeeper" ? 5.5 : 0;
            const score = progress * 0.62 +
...
function predictReceptionPoint(player, direction) {
...
function laneRisk(start, end, opponents) {
    let risk = 0;
    for (const opponent of opponents) {
        const projection = distanceToSegment(opponent.position, start, end);
        if (projection.along <= 0.08 || projection.along >= 0.94)
            continue;
        risk = Math.max(risk, clamp((2.2 - projection.distance) / 2.2, 0, 1));
    }
    return risk;
}
function forwardSpace(position, direction, opponents) {
    let space = 8;
    for (const opponent of opponents) {
...
function minimumDistance(position, players) {
    if (players.length === 0)
...
function distanceToSegment(point, start, end) {
    ...
    const along = lengthSquared <= 0.0001
        ? 0
        : clamp(((point.x - start.x) * dx + (point.z - start.z) * dz) /
            lengthSquared, 0, 1);
