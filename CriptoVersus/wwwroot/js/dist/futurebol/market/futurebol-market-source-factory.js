...
export function createFuturebolMarketSource(options) {
    ...
    if (mode === 'api') {
        return new ApiMarketSource(options.initialMarketSnapshot ?? createNeutralSnapshot(options));
    }
    throw new Error(`Modo de dados Futurebol não suportado: ${options.dataMode}`);
}
function createNeutralSnapshot(options) {
    return {
        sequence: 0,
