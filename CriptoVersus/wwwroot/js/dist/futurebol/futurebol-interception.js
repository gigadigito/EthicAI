 ...
 */
export function distanceToPassLine(playerPos, passStart, passEnd) {
    const dx = passEnd.x - passStart.x;
    const dz = passEnd.z - passStart.z;
    const lengthSquared = dx * dx + dz * dz;
    const along = lengthSquared <= 0.0001
        ? 0
        : clamp(((playerPos.x - passStart.x) * dx + (playerPos.z - passStart.z) * dz) /
            lengthSquared, 0, 1);
 ...
 */
export function buildInterceptionPlan(players, passStart, passEnd, intendedReceiverId, attackingTeam, seed, salt) {
    let bestCandidate = null;
    let bestDistance = INTERCEPTION_RANGE;
    for (const player of players) {
        if (player.id === intendedReceiverId)
            continue;
    ...
    if (projection.distance < INTERCEPTION_AUTO_RANGE) {
        ...
        const interceptionChance = clamp((INTERCEPTION_RANGE - projection.distance) / (INTERCEPTION_RANGE - INTERCEPTION_AUTO_RANGE) * 0.7, 0, 0.7);
 ...
 */
export function evaluateParry(shotProfile, shotEndZ, goalkeeperZ, seed, salt) {
    ...
    const parryChance = clamp(shotPower * 0.3 +
        shotOffCenter * 0.25 +
        gkPositioning * 0.2, 0.12, 0.52);
    const roll = deterministicUnit(seed, salt);
