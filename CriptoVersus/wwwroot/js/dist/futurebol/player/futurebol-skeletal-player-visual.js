...
export class FuturebolSkeletalPlayerVisual {
    constructor(B, scene, player, entries, teamVisual, logoMaterial) {
        this.B = B;
        this.scene = scene;
        this.player = player;
        this.teamVisual = teamVisual;
        this.logoMaterial = logoMaterial;
        this.rootMotionNodes = [];
         ...
         */
        this.poseRoot = new B.TransformNode(`futurebol-skeletal-pose-${player.id}`, scene);
        ...
        if (!this.skeletons.length) {
            throw new Error(`Skeleton ausente na instância ${player.id}.`);
        }
        this.root.metadata = { team: teamVisual.symbol };
        this.applyTeamMaterial();
        this.hideHumanHead();
        this.attachCoinHead();
        this.captureRootMotionNodes(entries.rootNodes);
        this.animationController =
            new FuturebolAnimationController(entries.animationGroups, player.id);
    }
    update(player, context, deltaSeconds) {
         ...
         */
        this.updateGoalkeeperDivePose(player, requested, deltaSeconds);
        this.root.position.set(player.position.x, player.position.y + this.diveOffsetY, player.position.z + this.diveOffsetZ);
     ...
     */
    updateGoalkeeperDivePose(player, requested, deltaSeconds) {
        const divingLeft = requested === "GoalkeeperDiveLeft";
        const divingRight = requested === "GoalkeeperDiveRight";
        const isDiving = player.role === "goalkeeper" &&
            (divingLeft || divingRight);
        const progress = clamp(player.actionProgress, 0, 1);
        /*
        ...
        const liftProgress = isDiving
            ...
            : 0;
        const desiredOffsetZ = side * travelProgress * 1.15;
        const desiredOffsetY = liftProgress * 0.34;
        /*
         * Rotação em X derruba o corpo lateralmente no campo.
         * Uma pequena rotação em Z evita um movimento rígido.
         */
        const desiredTiltX = side * travelProgress * 1.12;
        const desiredTiltZ = -side * liftProgress * 0.14;
        /*
         * Entrada rápida no mergulho e recuperação um pouco mais
         * suave para não parecer que o jogador teletransportou.
         */
        const responsiveness = isDiving ? 13 : 8.5;
        const safeDelta = clamp(deltaSeconds, 0, 0.1);
        const blend = safeDelta > 0
            ? 1 - Math.exp(-responsiveness * safeDelta)
            : 0;
        this.diveOffsetZ = lerp(this.diveOffsetZ, desiredOffsetZ, blend);
        this.diveOffsetY = lerp(this.diveOffsetY, desiredOffsetY, blend);
        this.diveTiltX = lerp(this.diveTiltX, desiredTiltX, blend);
        this.diveTiltZ = lerp(this.diveTiltZ, desiredTiltZ, blend);
        /*
    ...
    applyTeamMaterial() {
        const material = new this.B.StandardMaterial(`${this.player.id}-team`, this.scene);
    ...
    attachCoinHead() {
        ...
        const boneName = resolveCandidateName(bones.map(bone => bone.name), FUTUREBOL_PLAYER_ASSET.headBoneCandidates);
        const headBone = bones.find(bone => bone.name === boneName) ?? null;
        const sourceMesh = this.meshes.find(mesh => Boolean(mesh.skeleton));
        if (!headBone || !sourceMesh)
            throw new Error(`Bone de cabeça não resolvido para ${this.player.id}.`);
        const anchor = this.B.MeshBuilder.CreateBox(`${this.player.id}-coin-bone-anchor`, { size: 0.001 }, this.scene);
        anchor.isPickable = false;
        anchor.attachToBone(headBone, sourceMesh);
        anchor.position.y = FUTUREBOL_PLAYER_ASSET.coinLocalOffsetY;
        const coin = this.B.MeshBuilder.CreateCylinder(`${this.player.id}-coin-head`, {
        ...
        const frontSymbol = this.createCoinSymbolPlane(`${this.player.id}-coin-logo-front`, symbolDistance, 0);
        const backSymbol = this.createCoinSymbolPlane(`${this.player.id}-coin-logo-back`, -symbolDistance, Math.PI);
        frontSymbol.parent = anchor;
        backSymbol.parent = anchor;
        this.meshes.push(frontSymbol, backSymbol);
    }
    createCoinSymbolPlane(name, z, rotationY) {
        const symbol = this.B.MeshBuilder.CreatePlane(name, { size: FUTUREBOL_PLAYER_ASSET.coinSymbolSize }, this.scene);
        symbol.position.z = z;
    ...
    captureRootMotionNodes(rootNodes) {
        const nodes = rootNodes.flatMap(root => [
            root,
            ...root.getChildTransformNodes(false)
        ]);
        const name = resolveCandidateName(nodes.map(node => node.name), FUTUREBOL_PLAYER_ASSET
