...
export const FUTUREBOL_ANIMATION_MAP = {
    Idle: {
...
export function selectFuturebolAnimation(player, context) {
    /*
     * Resultado da jogada.
     */
    if (context.phase === "Outcome" &&
        context.outcome) {
        const scoredGoal = context.outcome === "Goal";
        const playerWon = scoredGoal &&
            player.team === context.activeTeam;
        const playerLost = scoredGoal &&
            player.team !== context.activeTeam;
        if (playerWon &&
     ...
     */
    const goalkeeperIsDiving = player.role === "goalkeeper" &&
     ...
     */
    if (context.ballOwnerId === player.id &&
...
export function resolveAnimationName(names, candidates) {
    const normalized = new Map(names.map(name => [
...
export function resolveCandidateName(names, candidates) {
    ...
    for (const name of names) {
        ...
        const matchesCandidate = candidates.some(candidate => normalizedName.endsWith(normalizeAnimationName(candidate)));
        if (matchesCandidate) {
            return name;
        }
    }
