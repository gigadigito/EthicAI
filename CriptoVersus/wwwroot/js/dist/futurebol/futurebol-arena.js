...
export class FuturebolArena {
    constructor(B, scene, quality) {
        ...
        this.scene = scene;
        this.detailMeshes = [];
        this.seatInstanceCount = 0;
        this.crowdInstanceCount = 0;
        this.floodlightInstanceCount = 0;
        ...
        console.info("[Futurebol][Arena] STADIUM_V2 loaded", {
            quality,
    ...
    createSteppedBowl(materials) {
        ...
        for (const side of [-1, 1]) {
            for (let row = 0; row < SIDE_TIER_COUNT; row++) {
                ...
                risers.push(this.box(`futurebol-side-step-riser-${side}-${row}`, 59, 0.62, 0.14, 0, deckY - 0.31, z - side * 0.48, materials.tier));
                for (const aisleX of SIDE_AISLES) {
                    aisles.push(this.box(`futurebol-side-stair-${side}-${row}-${aisleX}`, 0.62, 0.08, 0.92, aisleX, deckY + 0.12, z, materials.aisle));
                }
            }
        }
        for (const side of [-1, 1]) {
            for (let row = 0; row < END_TIER_COUNT; row++) {
                ...
                risers.push(this.box(`futurebol-end-step-riser-${side}-${row}`, 0.14, 0.62, 31.2, x - side * 0.48, deckY - 0.31, 0, materials.tier));
    ...
    createInstancedSeatsAndCrowd(materials) {
        const darkSeats = [];
        const coolSeats = [];
        const warmSeats = [];
        const darkCrowd = [];
    ...
    createStadiumShell(materials) {
        ...
        const wallRibs = [];
        for (const side of [-1, 1]) {
            shell.push(this.box(`futurebol-side-back-wall-${side}`, 62, 7.6, 0.38, 0, 3.8, side * 22.1, materials.shell));
            shell.push(this.box(`futurebol-side-canopy-${side}`, 62, 0.28, 4.8, 0, 8.35, side * 20.25, materials.shell, side * 0.12));
            roofFascias.push(this.box(`futurebol-side-roof-fascia-${side}`, 60, 0.58, 0.32, 0, 7.65, side * 18.05, materials.concourse));
            for (const x of [-27, -13.5, 0, 13.5, 27]) {
                supports.push(this.box(`futurebol-side-roof-column-${side}-${x}`, 0.28, 4.6, 0.3, x, 5.75, side * 21.45, materials.concourse));
                supports.push(this.box(`futurebol-side-roof-brace-${side}-${x}`, 0.24, 4.5, 0.24, x, 6.25, side * 20.2, materials.aisle, side * 0.55));
            }
            for (let x = -28; x <= 28; x += 4) {
                wallRibs.push(this.box(`futurebol-side-wall-rib-${side}-${x}`, 0.18, 3.3, 0.18, x, 4.5, side * 21.82, materials.concourse));
            }
        }
        for (const side of [-1, 1]) {
            shell.push(this.box(`futurebol-end-back-wall-${side}`, 0.38, 6.5, 44, side * 31.1, 3.25, 0, materials.shell));
            shell.push(this.box(`futurebol-end-canopy-${side}`, 4.4, 0.28, 44, side * 29.9, 7.55, 0, materials.shell, 0, 0, -side * 0.08));
        }
    ...
    createConcourseTunnelsAndBoards(materials) {
        const concourse = [];
        const tunnels = [];
        const tunnelFrames = [];
        const boardBacks = [];
        ...
        const orangeBoards = [];
        for (const side of [-1, 1]) {
            concourse.push(this.box(`futurebol-side-concourse-${side}`, 59, 0.74, 0.72, 0, 4.25, side * 21.25, materials.concourse));
            const frontZ = side * 15.62;
            tunnels.push(this.box(`futurebol-player-tunnel-${side}`, 6.2, 2.75, 0.22, 0, 1.36, frontZ, materials.portal));
            tunnelFrames.push(this.box(`futurebol-player-tunnel-header-${side}`, 7, 0.3, 0.3, 0, 2.83, frontZ - side * 0.04, materials.cyan));
            for (const x of [-3.35, 3.35]) {
                tunnelFrames.push(this.box(`futurebol-player-tunnel-side-${side}-${x}`, 0.3, 3, 0.3, x, 1.4, frontZ - side * 0.04, materials.cyan));
            }
            for (const x of [-24, -16, -8, 8, 16, 24]) {
                boardBacks.push(this.box(`futurebol-side-ad-board-${side}-${x}`, 7.2, 0.82, 0.18, x, 0.58, side * 15.52, materials.portal));
                const target = x < 0 ? orangeBoards : cyanBoards;
                target.push(this.box(`futurebol-side-ad-light-${side}-${x}`, 5.7, 0.2, 0.12, x, 0.58, side * 15.4, x < 0 ? materials.orange : materials.cyan));
            }
        }
        for (const side of [-1, 1]) {
    ...
    createFloodlightBanks(materials) {
        const frames = [];
        const supports = [];
        const halos = [];
        const bulbs = [];
        for (const side of [-1, 1]) {
            for (const bankX of [-21, -7, 7, 21]) {
                frames.push(this.box(`futurebol-floodlight-frame-${side}-${bankX}`, 6.8, 1.38, 0.24, bankX, 7.16, side * 18.58, materials.concourse));
                halos.push(this.box(`futurebol-floodlight-halo-${side}-${bankX}`, 6.4, 1.12, 0.12, bankX, 7.16, side * 18.4, materials.lampHalo));
                for (const supportX of [-2.35, 2.35]) {
                    supports.push(this.box(`futurebol-floodlight-support-${side}-${bankX}-${supportX}`, 0.2, 1.7, 0.2, bankX + supportX, 7.92, side * 19.25, materials.aisle, side * 0.38));
                }
        ...
        const bulb = this.B.MeshBuilder.CreateBox("futurebol-arena-floodlight-bulbs", { width: 0.5, height: 0.3, depth: 0.16 }, this.scene);
    ...
    createSeatInstances(name, transforms, material) {
        const seat = this.B.MeshBuilder.CreateBox(name, { width: 0.66, height: 0.58, depth: 0.16 }, this.scene);
        seat.material = material;
        this.applyThinInstances(seat, transforms);
        this.seatInstanceCount += transforms.length;
    }
    createCrowdInstances(name, transforms, material, detail) {
        const spectator = this.B.MeshBuilder.CreateSphere(name, { diameter: 0.43, segments: 6 }, this.scene);
        spectator.material = material;
        this.applyThinInstances(spectator, transforms, detail);
        this.crowdInstanceCount += transforms.length;
    }
    applyThinInstances(mesh, transforms, detail = false) {
        const matrices = new Float32Array(transforms.length * 16);
        for (let index = 0; index < transforms.length; index++) {
            const transform = transforms[index];
            const matrix = this.B.Matrix.Compose(new this.B.Vector3(transform.scaleX ?? 1, transform.scaleY ?? 1, transform.scaleZ ?? 1), this.B.Quaternion.RotationYawPitchRoll(transform.yaw ?? 0, 0, 0), new this.B.Vector3(transform.x, transform.y, transform.z));
    ...
    pickSeatGroup(row, column, dark, cool, warm) {
    ...
    pickCrowdGroup(row, column, dark, cool, warm, light) {
    ...
    box(name, width, height, depth, x, y, z, material, rotationX = 0, rotationY = 0, rotationZ = 0) {
