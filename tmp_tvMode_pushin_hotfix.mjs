...
const CameraDirector = {
    resolveTarget(payload) {
        ...
        switch (payload.state) {
            case "FieldFocus":
                return { scale: baseFocusScale, x: "0%", y: "0%", hud: payload.hudFadeEnabled ? 0.2 : 1, hudScale: 0.96 };
            case "AttackFocus":
                return { scale: baseFocusScale + 0.08, x: "0%", y: "0%", hud: payload.hudFadeEnabled ? 0.08 : 1, hudScale: 0.92 };
            case "GoalFocus":
                return { scale: baseGoalScale + 0.04, x: "0%", y: "0%", hud: payload.hudFadeEnabled ? 0 : 1, hudScale: 0.88 };
            case "ReplayFocus":
                return { scale: Math.max(1.12, baseGoalScale), x: "0%", y: "0%", hud: payload.hudFadeEnabled ? 0 : 1, hudScale: 0.9 };
            case "ScoreboardFocus":
                return { scale: 1.03, x: "0%", y: "0%", hud: 0.92, hudScale: 1.01 };
...
export function initBroadcastCamera(rootId = "tv-broadcast-camera-shell") {
    ...
    const tick = (now) => {
        ...
        if (Math.abs(broadcastCameraState.target.scale - broadcastCameraState.current.scale) < 0.0005) {
            broadcastCameraState.current.scale = broadcastCameraState.target.scale;
        }

        if (Math.abs(broadcastCameraState.target.hud - broadcastCameraState.current.hud) < 0.002) {
            broadcastCameraState.current.hud = broadcastCameraState.target.hud;
        }

        if (Math.abs(broadcastCameraState.target.hudScale - broadcastCameraState.current.hudScale) < 0.002) {
            broadcastCameraState.current.hudScale = broadcastCameraState.target.hudScale;
        }

        if (percentDistance(broadcastCameraState.current.x, broadcastCameraState.target.x) < 0.02) {
            broadcastCameraState.current.x = broadcastCameraState.target.x;
        }

        if (percentDistance(broadcastCameraState.current.y, broadcastCameraState.target.y) < 0.02) {
            broadcastCameraState.current.y = broadcastCameraState.target.y;
        }
