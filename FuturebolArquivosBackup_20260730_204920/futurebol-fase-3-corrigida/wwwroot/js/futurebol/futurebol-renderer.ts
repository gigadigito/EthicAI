...
export class FuturebolRenderer {
    ...
    public constructor(
        ...
        this.engine = new B.Engine(this.canvas, quality !== "Low", {
    ...
    public applyQuality(quality: FuturebolQuality): void {
        ...
        this.applyRenderResolution(quality);

        for (const net of this.netMeshes)
            net.setEnabled(quality !== "Low");

        const anisotropy = quality === "High"
            ? 8
            : quality === "Medium" ? 4 : 1;
        ...
        const expectedMapSize = quality === "High" ? 2048 : 512;
        ...
        this.scene.imageProcessingConfiguration.contrast =
            quality === "High" ? 1.14 : 1.05;
        this.scene.imageProcessingConfiguration.exposure =
            quality === "High" ? 1.03 : 1;
    }

    public resize(): void {
        this.applyRenderResolution(this.quality);
    }
    ...
    private applyRenderResolution(
        ...
        /*
         * High inicia nítido mesmo em telas DPR 1 e acompanha telas
         * de alta densidade até 1.6x. Medium permanece na resolução
         * CSS nativa. Low reduz a resolução para preservar FPS.
         */
        const targetPixelRatio = quality === "High"
            ? clamp(devicePixelRatio, 1.25, 1.6)
            : quality === "Medium" ? 1 : 1 / 1.45;
