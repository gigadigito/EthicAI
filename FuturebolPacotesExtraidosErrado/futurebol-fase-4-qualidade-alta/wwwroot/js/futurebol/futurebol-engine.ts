...
export class FuturebolEngine {
    ...
    public async initialize(): Promise<void> {
        ...
        try {
            ...
            this.updateQualityStatus(this.options.quality);
            this.updateMatchHud();
    ...
    public async setQuality(
        ...
        await this.renderer.setVisualConfiguration(
            this.state.players,
            quality,
            this.options.playerVisual
        );
        ...
        this.fpsSamples = 0;
        this.fpsTotal = 0;
        this.updateQualityStatus(quality);
        this.updateMatchHud();
        this.log("preset gráfico alterado", {
            quality,
            rendering: this.renderer.renderDiagnostics()
        });
    }
    ...
    private updateMatchHud(): void {
        ...
        const rendering = this.renderer.renderDiagnostics();
        setText("futurebol-debug-quality-active", this.options.quality);
        setText(
            "futurebol-debug-render-size",
            `${rendering.width} × ${rendering.height}`
        );
        setText(
            "futurebol-debug-pixel-ratio",
            `${rendering.devicePixelRatio.toFixed(2)}×`
        );
        setText(
            "futurebol-debug-hardware-scaling",
            rendering.hardwareScalingLevel.toFixed(3)
        );
    ...
    private updateQualityStatus(
        ...
        setText(
            "futurebol-quality-active",
            quality.toUpperCase()
        );

        this.canvas.dataset.futurebolQuality = quality;
    }
